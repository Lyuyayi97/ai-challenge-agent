...
class VLMAgent(AgentBase):
    ...
    def capture_observation(self, width: int = 1280, height: int = 720) -> list[dict[str, Any]]:
        """Capture one first-person perception for a deterministic strategy.

        Strategies call this instead of rebuilding the perception plumbing.
        The image logging path is intentionally identical to ``run_step`` so
        that training screenshots remain available for review.
        """
        if self.tongsim is None or self.character_id is None:
            return []
        try:
            perception = self.tongsim.acquire_first_person_perception(
                self.character_id, width=width, height=height
            )
        except Exception as exc:
            logger.warning("capture_observation failed: {}", exc)
            return []
        image_b64 = perception.get("image")
        self._save_perception_image(image_b64)
    ...
    def _maybe_handle_piece_transfer(self, subject: Any) -> dict[str, Any] | None:
        ...
        stage_name = str(subject.get("stage") or subject.get("task_type") or "")
        ...
        cache_key = f"{piece_object_id}:{piece_loc}:{target_loc}:{target_rot}"
        ...
        target_xyz = self._location_to_xyz(target_loc)
        if target_xyz is None:
            logger.warning("Piece transfer target location is not a 3D point: {}", target_loc)
            return None
        target_loc = target_xyz
        rotation_xyz = self._rotation_to_xyz(target_rot)
        if rotation_xyz is None:
            logger.warning("Piece transfer rotation is invalid: {}", target_rot)
            return None
        ...
        try:
            ...
            try:
                still_in_hand, _ = self.tongsim.has_object_in_hand(self.character_id)
            except Exception:
                still_in_hand = False
            if still_in_hand:
                logger.warning("Puzzle piece {} still in hand after placement", piece_object_id)
                return None

            if stage_name == "jigsaw":
                view_loc = [target_loc[0] - 150.0, target_loc[1] - 120.0, 3.0]
    ...
    @classmethod
    def _location_to_xyz(cls, value: Any) -> list[float] | None:
        coerced = cls._coerce_location(value)
        if isinstance(coerced, dict):
            x = coerced.get("X", coerced.get("x"))
            y = coerced.get("Y", coerced.get("y"))
            z = coerced.get("Z", coerced.get("z"))
        elif isinstance(coerced, (list, tuple)) and len(coerced) >= 3:
            x, y, z = coerced[0], coerced[1], coerced[2]
        else:
            return None
        try:
    ...
    @staticmethod
    def _rotation_to_xyz(value: Any) -> list[float] | None:
        if isinstance(value, dict):
            roll = value.get("roll", value.get("Roll", 0.0))
            pitch = value.get("pitch", value.get("Pitch", 0.0))
            yaw = value.get("yaw", value.get("Yaw", 90.0))
        elif isinstance(value, (list, tuple)) and len(value) >= 3:
            roll, pitch, yaw = value[0], value[1], value[2]
        elif isinstance(value, str):
            numbers = re.findall(r"[-+]?\d*\.?\d+", value)
            if len(numbers) < 3:
                return None
            roll, pitch, yaw = numbers[0], numbers[1], numbers[2]
        else:
            return None
        try:
